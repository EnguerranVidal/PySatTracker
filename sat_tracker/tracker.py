...
class Tracker:
    ...
    def update_objects(self, numbers):
        self.tracked_objects = []
        self.tracked_indexes = []
        self.n_tracked_objects = 0
        for number in numbers:
            index = self.database.search_index(number)
            self.tracked_objects.append(number)
    ...
    def draw2D(self, times):
        ...
        for i in range(self.n_tracked_objects):
            line = ax.plot(Long[i, :], Lat[i, :], transform=ccrs.Geodetic(), c="red")
